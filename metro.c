...
//APPING1
//LOGIN : makendy.piton
//J'ai fait à la fois Dijkstra et Bellman-Ford
#include <stdlib.h>
#include <stdio.h>
#include <stdarg.h>
#include <string.h>
...
#define STATION_COUNT ((sizeof stations)/(sizeof *stations))
#define LEG_COUNT ((sizeof legs)/(sizeof *legs))
#define INFINI 10000
...
{
  for (unsigned s = 0; s < STATION_COUNT; ++s)
    if (strcasecmp(name, stations[s].name) == 0)
      return s;
  error(1, "Unknown station \"%s\".\n", name);
  return -1;
}
//------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------
//-------------------------Display of shortest route-----------------------------------------

unsigned find_time(unsigned src, unsigned dst)
{
  for (unsigned i = 0; i < LEG_COUNT; i++)
  {
    if(legs[i].src == src && legs[i].dst == dst)
      return legs[i].time;
  }
  return 0;
}

void print_shortest_road(unsigned src, unsigned dst, unsigned *father)
{
  if(src != dst)
  {
    print_shortest_road(src, father[dst], father);
    printf("(%u seconds)\n", find_time(father[dst],dst));
  }
  printf("%s\n", stations[dst].name);
}

//----------------------Bellman-Ford-------------------------------------------------
unsigned *bellman_ford(unsigned src, unsigned *father)
{
  unsigned * dist = malloc(sizeof(unsigned) * STATION_COUNT);
  bool change;
  unsigned d;
  ...
  {
    change = false;
    for (unsigned j = 0; j < LEG_COUNT; j++)
    {
      d = dist[legs[j].src] + legs[j].time;
      if(d < dist[legs[j].dst])
      {
        dist[legs[j].dst] = d;
        change = true;
        father[legs[j].dst] = legs[j].src;
      } 
    }
    if(change == false)
     break; 
...
//------------------------------Dijkstra-------------------------------------------------
unsigned minimum(unsigned *dist, unsigned *todo, unsigned todo_size)
{
  if(todo_size == 1)
    return 0;
  
  unsigned min = 0;
  for(unsigned i = 1; i < todo_size; i++)
  {
    if(dist[todo[min]] > dist[todo[i]])
      min = i;
  }
  return min;
}
unsigned *dijkstra(unsigned src, unsigned *father)
{
  unsigned todo_size = STATION_COUNT;
  unsigned x, d;
  unsigned *todo = malloc(sizeof(unsigned) * STATION_COUNT);
  unsigned *dist = malloc(sizeof(unsigned) * STATION_COUNT);
  for (unsigned i = 0; i < STATION_COUNT; i++)
  {
    dist[i] = INFINI;
  ...
  while(todo_size > 0)
  {
    x = minimum(dist, todo, todo_size);
    todo_size--;
    for (unsigned j = 0; j < LEG_COUNT; j++)
    {
      if(legs[j].src == todo[x])
      {
        d = dist[legs[j].src] + legs[j].time;
        if(d < dist[legs[j].dst])
        {
...
{
  if (argc != 3)
    error(1, "Pass two stations names as arguments.\n");

  unsigned src = find_station(argv[1]);
  unsigned dst = find_station(argv[2]);
  printf("src: %4d  %s\ndst: %4d  %s\n", src, argv[1], dst, argv[2]);

  puts("\n----------(Bellman-Ford)--------");
  unsigned *father1 = malloc(sizeof(unsigned) * STATION_COUNT);
  unsigned *dist1 = bellman_ford(src, father1);
  printf("(Bellman-Ford) Estimated time to connect %s to %s : %d\n", stations[src].name, stations[dst].name, dist1[dst]); 
  print_shortest_road(src, dst, father1);

  puts("\n----------(Dijkstra)--------");
  unsigned *father2 = malloc(sizeof(unsigned) * STATION_COUNT);
  unsigned *dist2 = dijkstra(src, father2);
  printf("(Dijkstra) Estimated time to connect %s to %s : %d\n", stations[src].name, stations[dst].name, dist2[dst]);
  print_shortest_road(src, dst, father2);

  // Compute and display the shortest route between src and dst.
  // Example:
  //  % ./metro_ref "le kremlin-bicêtre" "porte de la chapelle"
  //  src:  161  le kremlin-bicêtre
  //  dst:  276  porte de la chapelle
  //  Estimated time to connect "Le Kremlin-Bicêtre" to "Porte de la Chapelle": 1402 seconds
  //   - Le Kremlin-Bicêtre
  //     (39 seconds)
  //   - Maison Blanche
  //     (36 seconds)
